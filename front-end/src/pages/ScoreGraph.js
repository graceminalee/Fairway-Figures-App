...
const graphData = useMemo(() => {
    ...
    const parScore = round.total_par;
    const actualScore = round.total_shots;
    const scoreDifference = actualScore - parScore;
    const holesPlayed = round.holes_played || 18;

    return {
        courseName: round.course_name || `Round ${index + 1}`,
        scoreRelativeToPar: scoreDifference,
        totalShots: actualScore,
        totalPar: parScore,
...
return (
    ...
    {performanceSummary && (
        <div>
        {`${performanceSummary.totalRounds} Rounds | 
            Average Score: ${performanceSummary.averageScore > 0 ? '+' : ''}${performanceSummary.averageScore} 
